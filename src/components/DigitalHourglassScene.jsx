...
function Moon({ country }) {
    ...
    const { position, isNight } = useMemo(() => {
        ...
        if (!night) return { position: null, isNight: false };
        ...
        // Keep moon in center area (between particle areas)
        // Top particles: y > 10, Bottom particles: y < -15
        // Center area: -15 < y < 10, so we use y around -2 to 2
        
        const angle = (progress - 0.5) * Math.PI; // -PI/2 to PI/2
        
        return {
            position: [
                Math.sin(angle) * 25, // X: movement
                Math.cos(angle) * 3 - 2, // Y: center area (between -5 and 1)
                -35 // Z: distance (far away)
            ],
            isNight: true
        };
    }, [country]);
    ...
    const moonMaterial = useMemo(() => {
        ...
        // Convert grayscale to normal map
        const normalImageData = ctx.getImageData(0, 0, 512, 512);
        const normalData = normalImageData.data;
        for (let i = 0; i < normalData.length; i += 4) {
            const gray = (normalData[i] + normalData[i + 1] + normalData[i + 2]) / 3;
            normalData[i] = 128; // R
            normalData[i + 1] = 128; // G
            normalData[i + 2] = Math.max(0, Math.min(255, gray)); // B (height)
    ...
    return (
        <group ref={moonGroupRef} position={position}>
            <mesh ref={meshRef}>
                <sphereGeometry args={[2, 64, 64]} /> {/* Higher resolution for realism */}
                <primitive object={moonMaterial} attach="material" />
            </mesh>
            {/* Moon glow - soft ambient light */}
            <pointLight intensity={0.8} distance={60} color="#fbbf24" decay={2} />
            <pointLight intensity={0.3} distance={100} color="#ffffff" decay={2} />
        </group>
    );
}
